...
def fetch_market_inputs(
    ...
    # FRED risk-free rate
    rates = pdr.DataReader(rate_series, "fred", start, end)
    if rates.empty:
        raise ValueError(f"No FRED data returned for series: {rate_series}")
    rates.index = pd.to_datetime(rates.index).tz_localize(None)
    rates = rates.rename(columns={rate_series: "risk_free_rate_pct"})
