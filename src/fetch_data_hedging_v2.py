...
def black_scholes_price(S, K, T, r, sigma, option_type="call", q=0.0):
    ...
    q      : annual continuous dividend yield in decimal form
    """
    if T <= 0:
        if option_type == "call":
            return max(S - K, 0.0)
    ...
    if option_type == "call":
        return S * dividend_discount * _norm_cdf(d1) - K * rate_discount * _norm_cdf(d2)
    return K * rate_discount * _norm_cdf(-d2) - S * dividend_discount * _norm_cdf(-d1)


def black_scholes_delta(S, K, T, r, sigma, option_type="call", q=0.0):
...
def black_scholes_greeks(S, K, T, r, sigma, option_type="call", q=0.0):
    ...
    gamma = dividend_discount * nd1 / (S * sigma * sqrtT)
    vega = S * dividend_discount * nd1 * sqrtT

    if option_type == "call":
        delta = dividend_discount * Nd1
        theta = (
            -(S * dividend_discount * nd1 * sigma) / (2.0 * sqrtT)
            - r * K * rate_discount * _norm_cdf(d2)
            + q * S * dividend_discount * Nd1
        )
        rho = K * T * rate_discount * _norm_cdf(d2)
    else:
        delta = dividend_discount * (Nd1 - 1.0)
        theta = (
            -(S * dividend_discount * nd1 * sigma) / (2.0 * sqrtT)
            + r * K * rate_discount * _norm_cdf(-d2)
            - q * S * dividend_discount * _norm_cdf(-d1)
        )
        rho = -K * T * rate_discount * _norm_cdf(-d2)
...
def black_model_price(F, K, T, r, sigma, option_type="call"):
    """
    Black-76 style pricing for options on futures/forwards.
    """
...
def black_model_greeks(F, K, T, r, sigma, option_type="call"):
    """
    Returns futures/forward delta, gamma, vega, theta, rho for Black-76 pricing.

    Notes:
    - delta and gamma are with respect to the futures/forward price F
    - vega is per 1.00 change in volatility
    - theta is annualized
    - rho assumes F is held fixed with respect to rates
    """
    if option_type not in {"call", "put"}:
        raise ValueError("option_type must be 'call' or 'put'")
    ...
    if T <= 0 or sigma <= 0 or F <= 0 or K <= 0:
        if option_type == "call":
            ...
            delta = -discount if F < K else 0.0
        return delta, 0.0, 0.0, 0.0, -T * black_model_price(F, K, T, r, sigma, option_type)
    ...
    gamma = discount * nd1 / (F * sigma * sqrtT)
    vega = discount * F * nd1 * sqrtT
    price = black_model_price(F, K, T, r, sigma, option_type)
    theta = -(discount * F * nd1 * sigma) / (2.0 * sqrtT) + r * price
    rho = -T * price
...
def fetch_market_inputs(
    ...
    try:
        ...
        raise ImportError(
            "yfinance is required to fetch market inputs. "
            "Install it or use a synthetic/local dataset path instead."
        ) from exc
    ...
    merged["dividend_yield"] = float(dividend_yield)
        ...
        for dte in expiry_days:
            ...
            for K in strikes:
                ...
                for option_type in ("call", "put"):
                    # Use a continuous dividend yield when the daily inputs provide one.
                    price = black_scholes_price(S, K, T, r, sigma, option_type, q=q)
...
if __name__ == "__main__":
    ...
    print("\n=== MERGED DAILY INPUTS ===")
    print(merged_df[["date", "close", "vix_close", "risk_free_rate", "dividend_yield", "base_iv"]].head())
